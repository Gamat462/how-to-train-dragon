...
html = bangun(
  ...
  kunci_simpan="sql-mahir-l1-v2",
  eyebrow="SQL Mahir · Level 1 dari 5 · kerjakan setelah Modul M6",
  ...
  lede_paras=[
    ...
    "yang paling terkenal di dunia SQL, lima jenis JOIN, self join, <code>HAVING</code>, dan "
    "operasi himpunan untuk merekonsiliasi dua sistem. <strong>Tujuh kali kamu akan sengaja "
    "membuat layarmu merah</strong>, dan lima kali melihat SQL menerima kesalahanmu tanpa "
    "berkedip — karena yang kedua itulah yang sebenarnya mahal.",
    "Basis data latihannya kamu bangun sendiri di Hari 1, satu sel demi satu sel. "
    "<strong>Setiap angka dan setiap pesan error di halaman ini dijalankan sungguhan sebelum "
  ...
  footer_html=(
    '    <p>Kemajuanmu tersimpan otomatis di browser ini. Seluruh latihan memakai '
    '<code>senja.db</code> yang kamu bangun di Hari 1 — skrip lengkapnya juga ada sebagai '
    '<code>gen_final.py</code> di <code>data-dan-skrip/sql-mahir/</code> di paket zip.</p>\n'
    '    <p style="margin-top:10px"><strong>Dari mana angkanya.</strong> Tiap kotak keluaran di '
    'halaman ini disalin dari menjalankan kodenya sungguhan pada SQLite 3.45.1 dan pandas 3.0.5 '
    '— termasuk ketujuh pesan error, yang dipicu dulu sebelum ditulis. Tidak ada satu angka pun '
    'yang diketik dari ingatan.</p>'),
